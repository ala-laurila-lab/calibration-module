function T_absorbance = myGovardovskiiNomogram(wls,lambdaMax)
% Modified GovardovskiiNomogram in psychophysicstoolbox
% T_absorbance = myGovardovskiiNomogram(wls,lambdaMax), wls:wavelength
% lambdaMax: Lambda @ peak absorbance
...
for i = 1:nT
  theMax = lambdaMax(i);
        if (theMax > lmaxLow & theMax < lmaxHigh)
                ...
                if theMax==496
                    Abeta = 0.39;% adjusting for the beta band in mouse rod Lyubarski et al. 1999 & Wang et al. 2011    
                end
